...
def test_ga_plan_defines_current_pc_delivery_gate_and_deferred_hardware_gate():
    ...
    assert_contains_all(
        ...
        [
            ...
            "RK3588/board/real robot/field validation 是 GA 之后的硬件适配/现场验证，不是 deferred GA gate，也不阻塞 GA",
            "PC evidence 只能声称 PC-simulated GA passed",
            "不得声称 `real robot validated`、`board compatible`、`RK supported` 或 `field GA passed`",
...
def test_ga_plan_baseline_and_team_review_match_current_cli_state():
    ...
    assert_contains_all(
        ...
        [
            ...
            "不代表真实 DDS factories/adapters、PC E2E、板端兼容或真机闭环已完成",
            "GA 验收口径：PC gate pass 是 repo-local PC DDS emulation",
            "硬件/现场证据属于 post-GA validation",
            ...
            "读取 `--data-dir`",
            "默认探测 `<data-dir>/manifest.json`",
            "标准 `--data-dir val-data` 时为 `val-data/manifest.json`",
            "显式 `--manifest`",
            "deterministic effective manifest",
            "scene 名称",
            "per-scene sha256",
            "frame count",
            "first/last frame",
            ...
            "CLI smoke evidence latency schema 已完成",
            "top-level `latency` 包含 `capture_to_gaze_publish_ms` 和 `capture_to_botified_stdout_ms`",
            "`capture_to_botified_stdout_ms` 仍是 unavailable placeholder",
            "`capture_to_gaze_publish_ms` 只接受 finite numeric values",
            "publish >= frame",
            "delta <= 60_000ms",
            "huge cross-timebase deltas 标记为 invalid/unavailable 且不使 partial smoke failed",
            "`gaze_publish_hz` 从 accepted sample publish timestamps 计算",
            "至少两条 sample 且严格递增",
            "available 且低于 9Hz 是 segment-specific fail",
            "unavailable 不 fail",
            "top-level `evidence_summary` 汇总 manifest/runtime/head_state/gaze/Botified/latency evidence",
            "`latency_p95_p99` 仍在 `not_covered`",
            "`ga_gate_pass=false`",
            ...
            "scene `pci_stand`",
            "`frame_count=3`",
            "每段 head publisher `returncode=0`/`published=3`",
            "mapped_state 分别为 `stationary`/`moving`/`unknown`",
            ...
            "仍是 partial",
            "不证明 full PC gate、full event oracle、Botified 端到端业务、latency P95/P99、soak、fault matrix、release report、RK/board 或 real robot",
            "PC E2E",
            "release report",
            "真机 smoke",
            "closed-loop handoff",
            ...
            "manifest/oracle",
            "必要轻量 fault/latency/stability checks",
            ...
            "DDS discovery/real serialization over wire/QoS behavior 的 PC runtime path 测试仍未完成",
...
def test_ga_plan_keeps_native_runtime_loop_done_and_one_remaining_boundary():
    ...
    assert_contains_all(
        ...
        [
            ...
            "不能替代这个端到端 gate",
            "DDS discovery/real serialization over wire/QoS behavior 的 PC runtime path 测试仍未完成",
            "正式 full CLI+real server 编排的完整 PC local E2E GA gate、全量 `val-data` scene matrix、manifest/oracle、必要轻量 fault/latency/stability checks 仍未完成",
            "RK/board probe、真机 smoke/closed-loop handoff 属于 post-GA validation",
        ],
    )

    assert text.count("完整 PC 本地 DDS E2E GA gate 仍未完成") >= 1
    assert text.count("DDS discovery/real serialization over wire/QoS behavior 的 PC runtime path 测试仍未完成") == 1
...
def test_ga_plan_records_step5_native_participants_and_wrappers_without_claiming_full_pc_e2e():
    ...
    assert_contains_all(
        ...
        [
            ...
            "`tools/cli_local_e2e_manifest.py`",
            "读取 `--data-dir`",
            "默认探测 `val-data/manifest.json`",
            "实际为 `<data-dir>/manifest.json`",
            "也支持显式 `--manifest`",
            "manifest 缺失时生成 deterministic effective manifest",
            "scene 名称",
            "per-scene sha256",
            "frame count",
            "first/last frame",
            ...
            "head/image publisher wrappers",
            "partial smoke report",
            "`tools/run_cli_local_e2e.py` 支持 `--require-authoritative-manifest`",
            "manifest contract judgment 集中在 `tools/cli_local_e2e_manifest.py` shared helper",
            "并被 `tools/run_val_data_e2e.py` 复用",
            "避免 server replay 与 CLI runner rules drift",
            "CLI report 投影 `manifest_contract_required`、`manifest_contract_satisfied`、`manifest_contract_failure_reasons`、`oracle_evaluated=false` 和 `oracle_evaluation_passed=null`",
            "不带该 flag 时 generated manifest 仍可用于 partial smoke",
            "report 会记录 contract gap",
            "带该 flag 时，missing/generated/non-authoritative/invalid/unreadable manifest 会在启动 server/CLI/DDS 前 preflight fail",
            "invalid/unreadable file manifest report 保留 `manifest_source=file`、`manifest_path` 和 `manifest_unreadable_or_invalid` marker",
            "valid authoritative manifest + flag 仍只是 partial smoke pass",
            "不做 oracle evaluation/full PC gate",
            "live bounded CLI stdout/stderr collector",
            'additive `report["botified_stdout"]`',
    ...
    server_replay_manifest = line_containing(
        step5, "Step 5 server replay manifest contract evidence slice 已完成"
    )
    assert_contains_all(
        server_replay_manifest,
        [
            "`tools/run_val_data_e2e.py`",
            "`--manifest`",
            "`--require-authoritative-manifest`",
    ...
    assert_contains_all(
        ...
        [
            ...
            "显式 `--head-state-segments` 会解析并 reject 空/重复/非法值",
            "`start head publisher -> run image publisher -> wait head publisher`",
            "nested `head_state.required`",
            "`head_state.publisher_mode`",
            "`head_state.hz`",
            "`head_state.stale_count`",
            "`head_state.unknown_ratio`",
            "`head_state.segments`",
            "flat aliases `head_state_publisher_mode`",
            "`head_state_hz`",
            "`head_state_stale_count`",
            "`head_state_unknown_ratio`",
            "`head_state_segments`",
            "native head publisher JSON evidence",
            "mapped_state/mapped_valid/dds_valid",
            "segment-specific image/gaze failures",
            "为 null/None",
            "CLI smoke evidence latency schema 已完成",
            "top-level `latency` 包含 `capture_to_gaze_publish_ms` 和 `capture_to_botified_stdout_ms`",
            "`capture_to_botified_stdout_ms` 仍是 unavailable placeholder",
