...
- 可测试优先：PC 本地 DDS 端到端测试是 GA release gate，不是 demo。
...
包名是否从 `visual-events-server` 改为 `visual-events` 需要在实现第一步决定。GA 推荐改成 `visual-events`，因为同一个 wheel 将包含 server 和 CLI 两个运行入口；如果暂不改名，文档必须明确这是历史包名。
...
- `botified_output`：`semantic_events` -> `<botified>...</botified>` request frame；按 `event_id` 幂等；stdout 只允许 Botified frame。
...
- Server 慢、断线、超时、错误响应时，CLI 不崩溃、不无界排队、不刷 Botified。
...
- 工具异常退出时清理子进程。
- Report 包含 frame count、image Hz、server response Hz、gaze Hz、Botified event count、stdout pollution count、reconnect count、stale/invalid sample count。
...
- stdout 只包含 Botified frame；任何 debug/status JSON 到 stdout 都 fail。
...
| Botified stdout | semantic event -> request frame | 高频状态不进 stdout；escaping 正确；同 `event_id` 不重复 |
| PC local E2E | real DDS participant + synthetic `val-data` image/head state publisher -> real CLI -> real server -> real DDS gaze sink/stdout collector | 7 scene 全量；5 分钟稳定；report pass |
| Fault matrix | server down/restart、slow server、DDS down/restart、bad JPEG、Botified pipe close | CLI 不 crash；可恢复；不刷屏 |
| 真机逻辑单元 | 机器人系统镜像/板端 CLI unit + DDS loopback | DDS type/runtime 兼容；不触碰真实运控 |
| 真机场景 | live camera DDS、real server、CLI、gaze sink/shadow consumer、Botified | 30 分钟稳定；事件符合场景；gaze 只走 DDS |
...
`gaze_target.stale_ms` 和 `service.response_timeout_ms` 是两个不同计时器：前者决定多久内必须让 gaze target 失效，GA 默认 250ms；后者决定 WebSocket 请求多久未返回后关闭连接并重连，GA 建议 1000ms。长时间 E2E 工具可以覆盖更大的 response timeout，但正式 CLI runtime 不应等 30s 才让 gaze 失效。
...
- PC 本地 DDS E2E 是正式 release gate。
