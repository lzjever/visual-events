...
- PC 本地 DDS 仿真是当前阶段 delivery gate；pass/fail authority 是 repo-local PC DDS emulation，包括 synthetic DDS camera/head-state publishers、真实 runtime server/CLI、DDS gaze subscriber/stdout collector、required reports 和所有 PC tests/audits passing。当前 PC GA 只验收模拟 DDS target 正确性、时效性和失效语义，不声称真机/RK/现场通过。
...
Release/handoff 是交付审计层。GA 阶段只要求 Step 7 的 PC 本地模拟 runtime path 跑通，并保留直接验证核心路径的 PC local DDS E2E、必要轻量稳定性和 latency 检查；不要为 release report skeleton、handoff audit、full fault matrix、long soak 或 field/real robot gate 单独扩张代码、schema 或测试。
