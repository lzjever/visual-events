...
当前 repo 已完成 CLI Step 3A skeleton + Step 3B pure logic：CLI package、`visual-events-cli` entrypoint、配置 skeleton、`target_mapper` 纯逻辑和 `botified_output` 纯逻辑；并已完成 CLI unit core 的 `service_client` WebSocket wire client、`frame_pump` deterministic core/stale watchdog、Botified stdout bounded queue/drop/coalescing 与 BrokenPipe exception 单元核心。仍未完成 formal CLI runtime loop/main wiring、真实 DDS adapters、runtime coordinator 对 BrokenPipe 的 publish stale then nonzero exit 处理、PC 本地 DDS E2E tools、release/runtime 编排、真机 smoke 和 closed-loop handoff。
...
- 真实 DDS adapters：DDS image/head state/gaze target 的 runtime adapter、serialization/QoS construction 和板端 compatibility probe。
...
- CLI package、entrypoint、config skeleton、`target_mapper` 和 `botified_output` 纯逻辑已存在；`service_client` WebSocket wire client unit core、`frame_pump` deterministic unit core/stale watchdog、Botified stdout bounded queue/drop/coalescing 与 BrokenPipe exception unit core 已完成；仍需完成 formal CLI runtime loop/main wiring、真实 DDS adapters、runtime coordinator 对 BrokenPipe 的 publish stale then nonzero exit 处理、PC E2E tools、release/runtime 编排、真机 smoke 和 closed-loop handoff。
