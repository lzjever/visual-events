...
- Step 4 native runtime loop core/full-bridge wiring/fake harness/build include fix slice 已完成：`runtime_loop` core；`visual_events_dds_bridge_runtime_loop_harness` fake harness；full-bridge `visual_events_dds_bridge` 无参数路径进入 Unitree DDS runtime loop；ABI-only 路径仍 explicit fatal `dds_runtime_not_implemented`；stdout emitter latest-slot 输出 camera_jpeg/head_state；stdin 读取 canonical `gaze_target` JSONL 并经 backend 发布 DDS gaze；async backend fatal 不被 stdin 阻塞；shutdown late fatal 仍输出 fatal JSONL；full-bridge 构建显式传入 repo-local CycloneDDS C++ include dir。该 slice 不证明 PC 本地 DDS E2E tools、真实 DDS over-wire 端到端发布订阅 gate、release report、RK/board 或真机闭环。
- ABI harness 复用同一 core 产出 fake camera/head，作为 native JSONL ABI/runtime skeleton 的最小回归证据。

剩余缺口：

- PC 本地 DDS E2E tools 和真实 DDS over-wire 端到端发布订阅 gate 仍未完成：需要 synthetic camera/head publishers、真实 runtime server/CLI、DDS gaze subscriber/stdout collector，验证 `/camera/image/jpeg` -> bridge/CLI -> server -> `/visual_events/gaze_target` over wire 和 Botified frame。当前 native runtime loop core 和 full-bridge wiring 已完成，但不能替代这个端到端 gate。
- DDS discovery/real serialization over wire/QoS behavior、real serialization/QoS tests 和 release report 仍未完成；不能只有 fake/mock path、generator oracle、ABI harness、construction harness、runtime loop harness 或 init/EOF path。
...
- DDS topic/type/QoS 合同已固化在 schema 文档；DDS PC E2E/over-wire gate 和板端 compatibility probe 仍必须补齐，否则 PC 和真机 E2E 不等价。
