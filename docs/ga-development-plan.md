...
- native PC DDS over-wire test participants 和 Python native participant wrappers 已完成，但完整 PC 本地 DDS E2E GA gate 仍未完成：还需要 manifest reader/report、mock visual_state server、正式 CLI+real server 编排、真实 runtime server/CLI 和 stdout collector，验证 `/camera/image/jpeg` -> bridge/CLI -> server -> `/visual_events/gaze_target` over wire 和 Botified frame。当前 native runtime loop core 和 full-bridge wiring 已完成，但不能替代这个端到端 gate。
...
- manifest reader/report、mock visual_state server、正式 CLI+real server 编排、完整 PC local E2E GA gate、fault matrix、release report、RK/board 和真机仍未完成。
