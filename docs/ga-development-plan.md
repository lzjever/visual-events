...
这些阈值是 release/现场验收的默认硬 gate。单个场地需要更严格阈值时只能收紧，不能放宽为 GA pass。

GA 的 target dwell gate 是 750ms。GA release config/gate 必须显式设置并验证满足 750ms；当前 server 默认 500ms 不能被当作 GA pass 证据，除非后续计划明确决定调整默认值并补对应证据。
...
- DDS stack decision record 随 Step 1 冻结；Step 4 缺少 Head/Gaze generated type support 接入、真实 bridge/factories、PC 安装/loopback smoke 或板端 probe 任一实际证据时，不能进入 PC/现场 E2E。
...
- 真实 DDS factories/adapters 仍未完成；当前默认 factories 只 fail-fast，不是真实 DDS runtime。
- Step 3 不实现真实 DDS factories/adapters；真实 DDS runtime adapters 属于 Step 4。
- 剩余是 Step 4 真实 DDS factories/adapters、PC 本地 DDS E2E tools、release/runtime 真跑、真机 smoke 和 closed-loop handoff。
...
- Step 4 Python JSONL bridge runtime integration slice 已完成，formal CLI bridge runtime opt-in slice 已完成：默认仍 fail_fast，不因 env 隐式切 bridge；显式 `[dds].runtime="bridge"`/`--dds-runtime bridge` 才走 `bridge_runtime_factories()`。真实 subprocess fake JSONL child + `bridge_runtime_factories()`/`run_runtime` 覆盖 camera/head -> service -> gaze stdin、logical camera、stale/cleanup、child nonzero/fatal；真实 DDS/C++/IDL/QoS/PC E2E/RK/真机仍未完成，真实 DDS factories/adapters、C++ native bridge binary、Head/Gaze generated type support 接入、real serialization/QoS tests、PC E2E tools、board/RK probe、release/runtime 真跑、真机 smoke/closed-loop handoff 仍未完成。
- Step 4 native DDS bridge build/probe foundation slice 已完成：`native/dds_bridge` CMake project 可构建 very small `visual_events_dds_bridge_probe`，链接 Unitree SDK2 和 video DDS `CameraFrame_` type props，固定 camera/head/gaze topic/type/QoS constants，并只向 stdout 输出单行 JSONL status frame（`protocol_version=1,type=status,code=probe_ok,message=...`）；`tools/build_dds_bridge.py` 拆分 foundation gate 和 full-bridge gate，foundation check/build/probe 只要求 SDK root、video publisher dir 和 `CameraFrame_` inputs，可在无 IDL generator 时成功，并把 `foundation_ready`、`visual_events_codegen_ready`、`visual_events_codegen_error` 写入 ignored `artifacts/` report。该 slice 只证明 camera/probe foundation，不实现 `HeadStateV1_`/`GazeTargetV1_` C++ type support，不实现 full bridge runtime loop，不证明真实 serialization/QoS、PC E2E、board/RK 或真机闭环。
- Step 4 DDS C++ idlc repo-local prepare/oracle hardening slice 已完成：`tools/prepare_dds_codegen_toolchain.py` 默认 pinned CycloneDDS/CycloneDDS-CXX 0.10.2，默认 repo-local ignored probe 输出目录为 `build/tools/cyclonedds-cxx-idlc-0.10.2/codegen_probe/`；`--check`/`--dry-run` 不下载、不构建、不写系统或用户目录，只验证路径、版本、显式 `idlc` 和 cxx backend 文本，并报告 `oracle_ok=false`；`--probe-codegen` 会实际调用 `idlc -l cxx -o <probe-output-dir> <probe-idl>`，遇到 `cannot load generator`/`cannot load generator cxx`、缺预期 `.hpp` 或缺预期 `.cpp` 必须 fail，即使 idlc return code 是 0。`--prepare` 显式准备 ignored repo-local CycloneDDS/CycloneDDS-CXX 0.10.2 C++ idlc toolchain，验证 tag commit、CMake Makefiles install、`bin/idlc-cxx` wrapper 和 installed artifacts，并自动复用同一个 codegen oracle；它不接受 `--idlc` 且不使用 `VISUAL_EVENTS_IDLC`。fake git/cmake/idlc 覆盖成功生成、0.11.0 fail、missing cxx generator 但 rc=0 fail、只生成 `.hpp` fail、clone/artifact/oracle failure。`tools/build_dds_bridge.py --check-full-bridge` 不再搜索 PATH，只接受显式 `--idlc` 或 `VISUAL_EVENTS_IDLC`，并复用同一个 codegen probe；缺显式 idlc、版本不是 0.10.2、backend 不能实际加载或缺 expected `.hpp/.cpp` 时 fail-fast，通过时报告 `visual_events_codegen_ready=true` 和 `oracle_ok=true`。该 slice 只证明 repo-local prepare 编排和 C++ codegen probe 会拒绝假阳性；外部源码/build/install/probe 输出不进 Git，不生成或接入 `HeadStateV1_`/`GazeTargetV1_` generated type support，不实现 full bridge/runtime/PC E2E/RK/真机。

剩余缺口：

- 真实 DDS runtime adapters：`DdsImageSubscriber` 订阅 `/camera/image/jpeg`，校验 `CameraFrame_` 字段并 latest-only；`DdsHeadStateSubscriber` 订阅头部状态并输出 `stationary|moving|unknown`；`DdsGazeTargetPublisher` 发布 `/visual_events/gaze_target`。
- 按 `docs/dds-stack-decision-record.md` 已冻结决策实现 C++ native DDS helper/bridge 的剩余工作：真实 DDS factories/adapters、Visual Events `HeadStateV1_`/`GazeTargetV1_` generated C++ type support 接入、full bridge runtime、real serialization/QoS 和 native bridge ABI implementation 仍未完成。
- Generated type support 接入、native bridge ABI implementation、real serialization/QoS tests 和 real serialization/QoS construction tests；不能只有 fake/mock path、generator oracle 或 probe-only path。
...
- PC E2E、PC 本地 DDS E2E tools、release/runtime 真跑、真机 smoke/closed-loop handoff。
...
- `val-data/manifest.json` 或等价 manifest reader：列出 scene 名称、scene sha256、frame count、fps、expected event timeline source/version、expected attention target timeline/rule（target label/track、allowed switch windows、no-target windows）；manifest 文件可位于 ignored `val-data/` 下，但 report 必须记录 `manifest_sha256` 和 manifest 副本摘要。
...
- 真实 DDS factories/adapters wiring、真实 serialization/QoS construction tests、release/runtime 真跑仍是剩余；真机 smoke 不能替代这些单元/集成测试。

验收：

- `uv run --group dev pytest -q` 全绿。
- CLI tests 不依赖真实机器人，但真实 DDS factories/adapters 的 serialization/QoS construction 必须被测试。
- 真机 smoke 不能替代 production runner/lifecycle unit core 和真实 DDS factories/adapters 的单元/集成测试。
...
- 标准命令必须显式传入 `--server-bin runtime/venv/bin/visual-events-server` 和 `--cli-bin runtime/venv/bin/visual-events-cli`；runner/report 校验 runtime bin 路径、wheel/runtime provenance 和 server/CLI exit code。使用 dev console script 或手工预启动 server 的结果不能标记 GA pass。
...
- GA release config/gate 必须显式验证 target dwell >=750ms；当前 server 默认 500ms 不能作为 GA pass 证据。
...
- Botified owner 完整产品闭环 artifact：至少一次 `Visual Events semantic event -> Botified 决策/回应动作 -> 冷却不重复招呼`；这是 Step 8/Step 9 handoff 证据，不是本 repo 的 Botified 会话逻辑实现。
...
- Handoff report 记录低频事件 false-negative/时序验收、scene-level duplicate greeting gate 和 Botified owner 完整产品闭环 artifact hash。
...
- Release handoff 必须包含 `val-data` manifest hash、expected attention target timeline/rule 验收、低频事件 false-negative/时序验收、runtime server/CLI 入口证明、head state 完整 GA 证明、Botified allowlist 证明、Botified owner 完整产品闭环 artifact、端到端 latency P95/P99 和 no-motion-SDK audit artifact/hash。
