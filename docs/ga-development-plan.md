...
- PC 本地 DDS 仿真是当前阶段 delivery gate；pass/fail authority 是 repo-local PC DDS emulation：本地开发 PC 模拟 robot 发送 DDS camera/head-state，真实 runtime server/CLI 跑通，DDS gaze subscriber/Botified stdout collector 收到预期输出，并完成 `val-data` PC E2E 与必要 PC tests/audits。当前 PC GA 只验收模拟 DDS target 正确性、时效性和失效语义，不声称真机/RK/现场通过。
- RK3588/board/real robot/field validation 是 GA 之后的硬件适配/现场验证，不是 deferred GA gate，也不阻塞 GA；PC evidence 只能声称 PC-simulated GA passed，不得声称 `real robot validated`、`board compatible`、`RK supported` 或 `field GA passed`。
...
- 可测试优先：PC 本地 DDS 端到端测试是 GA delivery gate，不是 demo；硬件/现场证据属于 post-GA validation。
...
Step 1 的 DDS stack decision record 已冻结；这只固化 SDK/bridge runtime choice、IDL codegen gate、native bridge ABI、runtime env vars、PC 安装/loopback smoke plan 和板端 probe plan，不代表真实 DDS factories/adapters、PC E2E、板端兼容或真机闭环已完成。GA 验收口径：PC gate pass 是 repo-local PC DDS emulation；硬件/现场证据属于 post-GA validation。
...
PC 本地 E2E 的定位是 GA delivery gate：证明 repo 内 CLI/server/DDS 合同在可复现数据上稳定，并作为 current phase pass / PC gate pass / GA pass 的 pass/fail authority。硬件/现场验证属于 post-GA validation，覆盖真实 camera DDS runtime/network、板端 DDS type/QoS compatibility、真实 head state topic/type/Hz/freshness，以及真实 head/motion consumer 或等价闭环验收。
...
- native PC DDS over-wire test participants、Python native participant wrappers、Step 5 manifest reader/report skeleton slice、mock visual_state server slice、CLI runtime + mock visual_state server integration slice 和 run_cli_local_e2e partial smoke runner slice 已完成，但完整 PC 本地 DDS E2E GA gate 仍未完成：还需要正式 full CLI+real server 编排覆盖 stdout/Botified、全量 `val-data` scene matrix、manifest/oracle、latency 和必要轻量 fault/stability checks，验证 `/camera/image/jpeg` -> bridge/CLI -> server -> `/visual_events/gaze_target` over wire 和 Botified frame。manifest skeleton 只做数据集身份和报告骨架，partial smoke runner 只证明 real server + CLI bridge runtime + DDS participant plumbing 和 observed CLI stdout 的 bounded Botified stdout checks，mock visual_state server 和 CLI runtime + mock server integration 只覆盖 CLI deterministic/failure-path，当前 native runtime loop core 和 full-bridge wiring 已完成，但都不能替代这个端到端 gate。
- DDS discovery/real serialization over wire/QoS behavior 的 PC runtime path 测试仍未完成；不能只有 fake/mock path、generator oracle、ABI harness、construction harness、runtime loop harness、init/EOF path 或手动 smoke。
...
- 正式 full CLI+real server 编排的完整 PC local E2E GA gate、全量 `val-data` scene matrix、manifest/oracle、必要轻量 fault/latency/stability checks 仍未完成；RK/board probe、真机 smoke/closed-loop handoff 属于 post-GA validation。
...
- PC DDS E2E/over-wire gate、真实 serialization/QoS tests 和必要 PC evidence 仍是剩余；真机 smoke 不能替代这些单元/集成测试，也不阻塞 GA。
...
Release/handoff 是交付审计层。GA 阶段只要求 Step 7 的 PC 本地模拟 runtime path 跑通，并保留直接验证核心路径的 PC local DDS E2E、必要轻量稳定性和 latency 检查；不要为 release report skeleton、handoff audit、full fault matrix、long soak 或 field/real robot validation 单独扩张代码、schema 或测试。
...
- 当前 PC gate evidence/hash：server baseline、CLI unit/integration、PC local E2E、必要轻量 fault/stability checks、runtime smoke、no-motion-SDK audit、val-data manifest hash、runtime server/CLI provenance、head state required-mode、Botified stdout allowlist、latency report。
...
- PC GA evidence 记录 `manifest_sha256`、model/runtime/config hash、端到端 latency P95/P99、head state 字段、synthetic DDS `dds_source_camera_name`、`logical_camera_name`/`camera.name` 和 PC DDS compatibility 结果。
...
| PC local E2E | real DDS participant + synthetic `val-data` image/head state publisher -> runtime CLI -> runtime server -> real DDS gaze sink/stdout collector | manifest 中所有 GA scene 全量；manifest hash 一致；必要轻量稳定性；latency P95/P99 达标；PC-simulated GA passed；当前数据集若为 7 个 scene 由 manifest 记录 |
...
- PC 本地 DDS E2E 是 GA delivery gate；真机/板端 DDS compatibility 和真实闭环验收是 GA 之后的硬件适配/现场验证，不是 deferred GA gate。
...
- DDS topic/type/QoS 合同已固化在 schema 文档；完整 DDS PC E2E/over-wire GA gate 仍必须补齐。板端 compatibility probe 属于 post-GA hardware validation；没有该验证不得声称 board compatible、RK supported 或真机 E2E 等价。
...
- GA evidence 必须包含 `val-data` manifest hash、expected attention target timeline/rule 验收、低频事件 false-negative/时序验收、runtime server/CLI 入口证明、head state PC gate required-mode 证明、Botified allowlist 证明、端到端 latency P95/P99 和 no-motion-SDK audit artifact/hash；Botified owner 完整产品闭环 artifact 属于 post-GA/field rollout evidence。
