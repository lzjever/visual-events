...
- 按 `docs/dds-stack-decision-record.md` 已冻结决策实现 C++ native DDS helper/bridge；真实 DDS factories/adapters、IDL codegen/toolchain implementation 和 native bridge ABI implementation 仍未完成。
- IDL codegen/native bridge ABI implementation，以及真实 serialization/QoS construction tests；不能只有 fake/mock path。
...
- Step 4 first slice/unit core 已完成：纯 Python SDK-neutral DDS adapter core/fakes 覆盖 QoS constants、CameraJpegMessage JPEG SOF dimension validation、fake image latest-only、Fake DDS adapters lifecycle unit core（start/close idempotent；close 后拒绝使用/重启）、HeadStateSample stationary/moving/unknown stale/future timestamp mapping、FakeDdsGazeTargetPublisher lifecycle、protocol names 和 no-motion/no-real-DDS import audit。真实 DDS factories/adapters、IDL codegen/toolchain implementation、native bridge ABI implementation、real serialization/QoS construction tests、板端 compatibility probe、PC E2E tools、release/runtime 真跑、真机 smoke/closed-loop handoff 仍未完成。
