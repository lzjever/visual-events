...
- Step 4 native full-bridge generated Head/Gaze C++ type-support compile/probe slice 已完成：`tools/build_dds_bridge.py --check --check-full-bridge --build --probe` 会运行 Head/Gaze IDL codegen oracle，CMake full-bridge 编译 `head_state_v1.hpp/.cpp` 和 `gaze_target_v1.hpp/.cpp`，native probe 检查 `CameraFrame_`、`HeadStateV1_`、`GazeTargetV1_` type props 并输出一行 JSONL status；Foundation 路径仍然 CameraFrame-only。该 slice 不实现 full bridge runtime loop，不证明真实 serialization/QoS、PC E2E、board/RK 或真机闭环。
- Step 4 native JSONL ABI/runtime skeleton slice 已完成：`visual_events_dds_bridge` target 存在，`--probe` 单行 JSONL status；不带参数运行只 explicit fatal `dds_runtime_not_implemented`，不假装真实 DDS runtime；`visual_events_dds_bridge_abi_harness` 复用同一 ABI core 产出 fake camera/head，并消费 Python canonical `gaze_target`；parser 严格 canonical fields + state 闭集。该 slice 不实现 Unitree Channel reader/writer runtime，不证明真实 DDS backend/factories/adapters、real serialization/QoS、PC E2E、board/RK 或真机闭环。
- Step 4 native generated DDS type/ABI mapping construction slice 已完成：`CameraFrame_ -> CameraJpegFrame`；foundation camera mapping 可无 generated Head/Gaze；full-bridge mapping harness 编译 generated Head/Gaze，并验证 `HeadStateV1_ -> HeadStateFrame`、`GazeTargetFrame -> GazeTargetV1_`、camera/head/gaze field mapping、head state derived stationary/moving/unknown、gaze valid/state consistency 和 finite/range checks。mapping harness 不启 DDS 网络、不调用 Unitree Channel；该 slice 不证明 real serialization/QoS、真实 DDS runtime/PC E2E/RK/真机仍未完成。
...
- 按 `docs/dds-stack-decision-record.md` 已冻结决策实现 C++ native DDS helper/bridge 的剩余工作：真实 DDS backend/factories/adapters、Unitree Channel reader/writer runtime、real serialization/QoS construction tests 仍未完成。
- Unitree Channel reader/writer runtime、full bridge runtime wiring、real serialization/QoS tests 和 real serialization/QoS construction tests；不能只有 fake/mock path、generator oracle、ABI harness 或 probe-only path。
...
- PC E2E、PC 本地 DDS E2E tools、release/runtime 真跑、RK/board probe、真机 smoke/closed-loop handoff。
