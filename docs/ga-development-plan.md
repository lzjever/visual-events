...
当前 repo 已完成 CLI Step 3A skeleton + Step 3B pure logic：CLI package、`visual-events-cli` entrypoint、配置 skeleton、`target_mapper` 纯逻辑和 `botified_output` 纯逻辑。尚未完成 CLI runtime loop、service_client、frame_pump、真实 DDS adapters、Botified stdout writer/backpressure、PC 本地 DDS E2E tools、release/runtime 编排、真机 smoke 和 closed-loop handoff。
...
### Step 3：实现正式 CLI core（3A skeleton/3B pure logic 已完成，runtime loop 剩余）
...
- stdout writer 使用 bounded queue/drop/coalescing；slow stdout 不得阻塞 gaze stale watchdog。BrokenPipe 行为固定为：尽力发布一次 `valid=false,state=stale` 后受控非 0 退出，由 Botified supervisor 重启。
...
- CLI package、entrypoint、config skeleton、`target_mapper` 和 `botified_output` 纯逻辑已存在；仍需完成 runtime loop、真实 DDS adapters、Botified stdout writer、PC E2E tools 和 release/runtime 编排。
