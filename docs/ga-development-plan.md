...
当前 repo 已完成 CLI Step 3A skeleton + Step 3B pure logic：CLI package、`visual-events-cli` entrypoint、配置 skeleton、`target_mapper` 纯逻辑和 `botified_output` 纯逻辑；并已完成 CLI unit core 的 `service_client` WebSocket wire client、`frame_pump` deterministic core/stale watchdog、Botified stdout bounded queue/drop/coalescing 与 BrokenPipe exception 单元核心。仍未完成 Step 3 formal CLI runtime loop/main wiring、runtime coordinator 对 BrokenPipe 的 publish stale then nonzero exit 处理、Step 4 真实 DDS adapters、PC 本地 DDS E2E tools、release/runtime 编排、真机 smoke 和 closed-loop handoff。
...
### Step 3：实现正式 CLI core（3A skeleton/3B pure logic 和 unit core 已完成，runtime loop/main wiring 剩余）
...
- Formal CLI runtime loop/main wiring：把已测 unit core 接入正式 CLI lifecycle、config、metrics/logging 和 shutdown/reconnect 编排。
- Runtime coordinator/main wiring 依赖 Step 4 的 DDS adapter interfaces；Step 3 只负责正式 runtime loop/coordinator/main wiring，不实现真实 DDS adapters。
- Runtime coordinator 对 BrokenPipe 的 publish stale then nonzero exit 处理：BrokenPipe exception 已有 unit core，但正式 runtime 仍需尽力发布一次 `valid=false,state=stale` 后受控非 0 退出，由 Botified supervisor 重启。
...
- CLI package、entrypoint、config skeleton、`target_mapper` 和 `botified_output` 纯逻辑已存在；`service_client` WebSocket wire client unit core、`frame_pump` deterministic unit core/stale watchdog、Botified stdout bounded queue/drop/coalescing 与 BrokenPipe exception unit core 已完成；仍需完成 Step 3 formal CLI runtime loop/main wiring、runtime coordinator 对 BrokenPipe 的 publish stale then nonzero exit 处理、Step 4 真实 DDS adapters、PC E2E tools、release/runtime 编排、真机 smoke 和 closed-loop handoff。
