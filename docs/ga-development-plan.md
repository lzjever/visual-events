...
Step 1 仍未完成 DDS stack decision record：尚未实际确定 SDK/bridge runtime choice、IDL codegen 流程、native bridge ABI、runtime env vars、PC 安装方式和板端 probe 命令。

当前 repo 已完成 CLI Step 3A skeleton + Step 3B pure logic + RuntimeCoordinator/main wiring unit core：CLI package、`visual-events-cli` entrypoint、配置 skeleton、`target_mapper` 纯逻辑和 `botified_output` 纯逻辑；并已完成 CLI unit core 的 `service_client` WebSocket wire client、`frame_pump` deterministic core/stale watchdog、RuntimeCoordinator/main wiring unit core、main runtime_runner 注入和默认 Step 4 DDS adapters fail-fast、exact stale deadline、slow Botified drain 不阻塞 stale、Botified stdout bounded queue/drop/coalescing 与 BrokenPipe exception 单元核心、BrokenPipe publish stale then nonzero unit core。production runner/lifecycle 依赖 Step 4 真实 DDS adapters 实例化 RuntimeCoordinator，并接入正式 start/run/shutdown/reconnect、metrics/logging；剩余是 Step 4 真实 DDS adapters、PC 本地 DDS E2E tools、release/runtime 编排、真机 smoke 和 closed-loop handoff。

当前 repo 已完成 Step 4 first slice/unit core：纯 Python SDK-neutral DDS adapter core/fakes 位于 `visual_events_cli.dds.qos`、`visual_events_cli.dds.types`、`visual_events_cli.dds.protocols`、`visual_events_cli.dds.fake`，覆盖 QoS constants、CameraJpegMessage JPEG SOF dimension validation、fake image latest-only、HeadStateSample stationary/moving/unknown stale/future timestamp mapping、FakeDdsGazeTargetPublisher lifecycle、protocol names，并有 no-motion/no-real-DDS import audit；该层不 import 真实 DDS SDK/ML/运控依赖。Step 4 真实 DDS runtime adapters、SDK/bridge decision record、IDL codegen/native bridge ABI、real serialization/QoS construction tests、板端 compatibility probe、PC E2E tools、release/runtime 编排、真机 smoke/closed-loop handoff 仍未完成。
...
- 短遮挡或 track 切换期间，同一物理人不得产生新的招呼型事件序列；即使 `track_id` 变化，也必须通过 scene/person label 的验收 oracle 发现重复招呼风险。
...
### Step 1：固化合同文档和 schema（主要产物已完成，DDS stack decision record 剩余）
...
- DDS stack decision record：SDK/bridge 名称和版本、IDL codegen 命令、native bridge ABI、runtime env vars、PC 安装方式、板端 probe 命令。当前尚未实际确定 DDS SDK/bridge runtime choice，不能把该项视为完成。
...
- DDS stack decision record 随 Step 1 冻结；缺少 SDK/bridge 版本、IDL codegen、ABI、env vars、PC 安装或板端 probe 任一字段时，不能进入 PC/现场 E2E。
...
- production runner/lifecycle 依赖 Step 4 真实 DDS adapters 实例化 RuntimeCoordinator，并接入正式 start/run/shutdown/reconnect、metrics/logging。
- Step 3 不实现真实 DDS adapters；生产 runner 接真实 DDS adapter 属于 Step 4+runtime lifecycle 收口。
- 剩余是 Step 4 真实 DDS adapters、PC 本地 DDS E2E tools、release/runtime 编排、真机 smoke 和 closed-loop handoff。
...
- `FakeDdsGazeTargetPublisher` lifecycle 可重复 start/stop，并保护未 start publish。
...
- SDK/bridge decision record：SDK/bridge 名称和版本、IDL codegen 命令、native bridge ABI、runtime env vars、PC 安装方式、板端 probe 命令。
- IDL codegen/native bridge ABI，以及真实 serialization/QoS construction tests；不能只有 fake/mock path。
...
- PC E2E tools、release/runtime 编排、真机 smoke/closed-loop handoff。
...
- Report 必须按进入、路过、靠近、停留、挥手等低频事件记录 expected occurrence、允许延迟窗口和负例不得触发，并包含 scene-level 断言：同一物理人短暂 lost/恢复期间不得产生新的招呼型事件序列；如果 `track_id` 变化也要通过 scene/person label 判定。
...
- 7 个 `val-data/` scene 全量通过，且 manifest 中 scene 名称、sha256、frame count、fps、expected event timeline source/version、expected attention target timeline/rule 与 report 一致。
...
- Scene-level duplicate greeting gate 通过：同一物理人短暂 lost/恢复期间不得产生新的招呼型事件序列；`track_id` 变化时也必须通过 scene/person label 发现。
...
- 短遮挡/track 切换不得重复招呼：同一物理人短暂 lost/恢复期间不得产生新的招呼型事件序列；`track_id` 变化时也必须通过 scene/person label 发现。
...
| Semantic event oracle | 进入、路过、靠近、停留、挥手、负例 scene、短遮挡/track 切换 | 每类事件 expected occurrence 和允许延迟窗口通过；负例不得触发；同一物理人短暂 lost/恢复期间不得产生新的招呼型事件序列 |
...
- CLI package、entrypoint、config skeleton、`target_mapper` 和 `botified_output` 纯逻辑已存在；`service_client` WebSocket wire client unit core、`frame_pump` deterministic unit core/stale watchdog、RuntimeCoordinator/main wiring unit core、main runtime_runner 注入和默认 Step 4 DDS adapters fail-fast、exact stale deadline、slow Botified drain 不阻塞 stale、Botified stdout bounded queue/drop/coalescing 与 BrokenPipe exception unit core、BrokenPipe publish stale then nonzero unit core 已完成。production runner/lifecycle 依赖 Step 4 真实 DDS adapters 实例化 RuntimeCoordinator，并接入正式 start/run/shutdown/reconnect、metrics/logging；剩余是 Step 4 真实 DDS adapters、PC E2E tools、release/runtime 编排、真机 smoke 和 closed-loop handoff。
- DDS contract/schema Step 1 主要产物已完成；DDS stack decision record 仍未完成，必须实际确定 SDK/bridge runtime choice、IDL codegen、native bridge ABI、runtime env vars、PC 安装方式和板端 probe 命令。
...
- Step 4 first slice/unit core 已完成：纯 Python SDK-neutral DDS adapter core/fakes 覆盖 QoS constants、CameraJpegMessage JPEG SOF dimension validation、fake image latest-only、HeadStateSample stationary/moving/unknown stale/future timestamp mapping、FakeDdsGazeTargetPublisher lifecycle、protocol names 和 no-motion/no-real-DDS import audit。真实 DDS runtime adapters、SDK/bridge decision record、IDL codegen/native bridge ABI、real serialization/QoS construction tests、板端 compatibility probe、PC E2E tools、release/runtime 编排、真机 smoke/closed-loop handoff 仍未完成。
