...
- 按 `docs/dds-stack-decision-record.md` 已冻结决策实现 C++ native DDS helper/bridge 的剩余工作：真实 DDS factories/adapters、full bridge runtime、real serialization/QoS 和 native bridge ABI implementation 仍未完成。
- Full bridge runtime wiring、native bridge ABI implementation、real serialization/QoS tests 和 real serialization/QoS construction tests；不能只有 fake/mock path、generator oracle 或 probe-only path。
