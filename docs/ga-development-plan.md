...
- native PC DDS over-wire test participants 已完成，但完整 PC 本地 DDS E2E GA gate 仍未完成：还需要 Python runner/wrappers、manifest reader/report、mock visual_state server、正式 CLI+real server 编排、真实 runtime server/CLI 和 stdout collector，验证 `/camera/image/jpeg` -> bridge/CLI -> server -> `/visual_events/gaze_target` over wire 和 Botified frame。当前 native runtime loop core 和 full-bridge wiring 已完成，但不能替代这个端到端 gate。
...
- `tools/publish_test_dds_images.py`：native image publisher 的 runner/wrapper，从 `val-data/` 或 JPEG 目录按 10Hz 发布 DDS image；支持 loop、jitter、drop、invalid JPEG、timestamp skew。
- `tools/publish_test_head_state.py`：native head publisher 的 runner/wrapper，发布 `stationary|moving|unknown` 头部状态。
- `tools/subscribe_test_gaze_targets.py`：native gaze subscriber 的 runner/wrapper，订阅 gaze target，写 ignored JSONL，校验 rate、schema、stale、坐标范围和 invalid sample。
...
- Python runner/wrappers、manifest reader/report、mock visual_state server、正式 CLI+real server 编排、完整 PC local E2E GA gate、fault matrix、release report、RK/board 和真机仍未完成。
- 已完成的 native participants 只证明 PC loopback DDS over-wire participants 可用；不能替代正式 CLI + real server + `val-data` manifest 的完整 GA gate。
