...
Machine-readable authority 是 `artifacts/memory-identity-handoff/report.json`。
...
uv run python tools/generate_memory_teaching_evidence.py \
  ...
  --out artifacts/memory-identity-handoff-evidence \
  --strict-source-ok
```

Human demo 是 `artifacts/memory-identity-handoff-evidence/index.html`。旧的 `artifacts/memory-identity-evidence`、`artifacts/memory-teaching-*`、`artifacts/memory-identity-*-check` 只是历史/阶段性本地 ignored artifact，不要作为 active handoff/demo 入口。
