...
本文定义以下主题的当前产品和开发要求：`Identity Overlay`、`teach_person` server-side auto merge、`identify-current`、memory/teaching core contract、`.transcript` 交互输入、identity evidence、CLI current visual snapshot。旧资料在 `docs/legacy/`，仅供追溯，不作为当前设计约束。

公开 demo 入口以 [Real Model Evidence Demo Development Plan](real-model-evidence-demo-development-plan.md) 为准：visual demo 使用 `tools/run_visual_demo.py`，memory / identity / teaching demo 使用 `tools/run_memory_demo.py`。其他历史脚本和 ignored artifact 目录只保留追溯价值，不作为 active demo 主路径。

## 1. 定位

本计划覆盖身份、记忆和示教的完整产品闭环：
...
- 不测试测试：测试核心行为和合同，不做像素级截图回归、测试工具内部测试或报告平台。
...
- `AppMemoryService` 已有低频 background recognition、known person 查询、anonymous / familiar unknown 查询、scene 查询、memory event cooldown 和短期 identity overlay。
...
- demo/report 已覆盖 identity overlay、teach auto-merge、identify-current、event identity enrichment、CLI current visual snapshot、object unsupported no-write 和 active API payload 摘要。

剩余缺口：

- 还需要更完整的端到端 demo / 长时间运行证据，证明真实模型、真实摄像头和现场光照下的稳定性。
- familiar unknown 的长期质量仍可继续优化，例如更稳的时长统计、去重、阈值和过期策略。
- RK / 现场部署、运维观测和性能长期趋势属于后续产品化工作，不属于当前 identity overlay 范围。
...
```json
{
  ...
  "evidence": {
    ...
    "request_frame_ref": "memory_frame:front:100:1780000000000"
  }
}
```
...
除请求格式错误外，以上都作为 HTTP 200 业务状态返回。请求 schema 违法仍使用 4xx。每个 person item 仍使用统一 `identity_context.status=known_person|familiar_unknown|unknown|pending|unavailable`。`identify-current` response 不返回 public `track_id`、bbox 或 keypoints；需要定位调试时只在 evidence/report 中使用 `source_frame_ref`、`request_frame_ref` 等引用。
...
### 5.7 Memory/Teaching Core Requirements

本区块定义 memory / teaching 的当前单一路径要求。不新增第二套产品范围、报告平台或人工审核流程。
...
- teach / resolve / identify active API 必须按 `stream_ref + camera` 选择 request-arrival snapshot / hot buffer；缺失、未知、过期或 stale snapshot 不回退 camera-only 最新帧，不写库。demo report 记录 `request_frame_ref`、`source_frame_ref`、frame timestamp、snapshot `observed_at_ms`、TTL 和 N-of-M stability 结果。
...
- embedding provenance 随 person/scene + embedding 同事务持久提交，最小字段包括 `source_track_ref`、`source_frame_ref`、`source_crop_ref`、`crop_hash`、`resolved_target_ref`、`resolution_reason`、`embedding_type`、`embedding_model`、`embedding_version`、`embedding_dim`。
- third-person 写库必须能机器证明写入 B 不是 A：至少记录 `introducer_ref`、`resolved_target_ref`、`stored_embedding_source_track_ref`、`stored_crop_frame/path`、`stored_crop_hash`、`profile.display_name` 和 pose pointing evidence；断言 `stored_embedding_source_track_ref == resolved_target_ref` 且 `stored_embedding_source_track_ref != introducer_ref`。
- teach/write embedding、recognition retrieval、event recall 和 `identify-current` 都走 bounded worker / bounded retrieval / cooldown，不阻塞 10Hz detection/tracking/gaze；demo report 记录 queue/backpressure、timeout、max tracks per tick、rate limit、worker backlog 和主链路 latency 摘要。

Demo/report 最小内容：

- demo 使用同 stem `.transcript` + `.jpeg/.jpg` interaction inventory，不使用 `des.txt`。
- machine-readable report 至少能关联：`source_text_path`、`source_image_path`、`source_frame_ref`、`request_frame_ref`、source/crop path 或 crop hash、API response record、event record、Botified frame / CLI projection record。
- visual demo 只展示通用视觉推理结果：人物框、pose / attention、事件、真实视觉模型标识和全量 val-data 覆盖。
- memory demo 展示记忆 / 示教 / 身份相关能力：自我介绍记住、场景示教、第三人称指向示教、匿名熟客出现、identity overlay、event identity_context、identify-current、CLI current visual snapshot、teach auto merge，以及真实 face / pose / scene 模型标识。缺少 optional identity evidence 显示 `not_present`；缺 source frame/text/image/crop 这类必要输入则明确报错。
...
不要新增第二套字段或让 CLI 重新计算身份。

### 6.4 CLI notification coalescing
...
这只是 CLI 侧通知去重，不改变 server 的事件事实输出。
...
- 如果 `scene_context.target_reacquired` 或 EventEngine 状态表明 old track -> new track，overlay 可以复制旧 track 的未过期身份到新 track。
...
- 禁止 `track_id`、`bbox`、`point_uv`、`source_frame`、`source_frame_ref`、`request_frame_ref`。
...
### 7.11 Real Model Demo Evidence

公开用户入口只保留两个 real-model demo wrapper：
...
`tools/run_visual_demo.py`：

- 输出 `artifacts/demo/visual/index.html`。
- 展示 detection、tracking、attention、semantic events、no-event / no-engage reason。
- 如果同一套 visual state 已有 identity summary，renderer 可以显示短摘要；identity 验证仍以 memory demo 为准。

`tools/run_memory_demo.py`：

- 输出 `artifacts/demo/memory/index.html`。
- 是 memory / identity / teaching 的唯一 human demo 入口。
- 展示 teach auto merge anonymous、identify-current result、event identity enrichment。
...
- 可以复用已有 renderer/helper，但不新增或推荐第二套用户可见 evidence CLI。

## 8. 开发步骤

1. 文档和 contract 收敛：本计划作为 identity / memory / teaching 的 active 设计。
...
13. 更新 `tools/run_visual_demo.py`、`tools/run_memory_demo.py` 和 CLI current visual snapshot demo。
14. 补核心 tests 和 deterministic evidence demo。
15. 同步更新 demo runner / API tests 中的 explicit merge 主路径，必须改为 `teach_person` auto merge；maintenance merge API 只保留单独测试。
...
- JPEG replay inventory：demo runner 按实际目录发现 `.jpeg` / `.jpg` 图像和 scene 目录。
- interaction inventory：只由同 stem `.transcript` 和同 stem 图像组成。
...
- `.transcript` 表示用户消息发生在对应参考图像附近的时间点。demo runner replay 到该图像附近时，使用最新 `visual_state.stream_ref` 和该文本手工构造 `teach_person` / scene teach / identify 相关 API payload，模拟用户当时对机器人说话。
- 不调用 LLM 自动解析 `.transcript`；测试中把文本手工映射为固定 request 模板，并在 report 中记录 `source_text_path`、`source_image_path`、`source_frame_ref` 和 `request_frame_ref`。
- demo runner 不修改 `val-data/`，不把 `val-data/`、模型、runtime DB 或 artifacts 加入 Git。
...
- DB schema 有 `observed_duration_ms DEFAULT 0`。
...
- merge 后不再输出已合并 anonymous 的 familiar event。
- 再次看到同一人输出 known_person_present。

### 9.3 Demo 验证

核心行为由 unit / integration tests 覆盖；用户可见 memory identity demo 只走 real-model wrapper：

```bash
uv run --extra inference python tools/run_memory_demo.py --data-dir val-data
```

Machine-readable demo report 是 `artifacts/demo/memory/report.json`；human demo 是 `artifacts/demo/memory/index.html`。报告只描述本次 demo 事实，避免外推到其他运行环境。

memory demo / report 需要证明：
...
Memory demo visual evidence 只用于人工查看。只断言关键字段存在，不做像素级验证。历史 ignored artifacts 不作为 active demo 入口。
...
它输出 `artifacts/demo/visual/index.html`。自然 replay 不一定触发 familiar / teach / identify-current，因此 identity 验证以 memory demo 为准。
...
- memory demo 能人工看到 event identity、current identity、CLI current visual snapshot、identify-current、teach auto merge。
...
- 复用现有 `FrameCache`、`MemoryFrameSnapshot`、known/anonymous store 和 memory event cooldown。
- 当前 `teach_person` anonymous 409 分支和新方向冲突，需要替换为自动 merge。
- Event enrichment 应在 memory/app attach 阶段做，不放进 EventEngine。

QA review：

- 验收分两层：核心行为用 pytest，demo 用 deterministic artifacts。
- 全量 val-data 普通 replay 只覆盖 visual demo；identity 验证走 memory demo。
- 不测试测试工具，不做像素级验证，不引入额外报告主线。
