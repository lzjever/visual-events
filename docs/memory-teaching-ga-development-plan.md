...
本阶段目标是把“记忆和示教”作为稳定可发布能力交付：用户通过 agent 明确示教当前画面中的人或整图场景，服务端保存记忆；后续 replay 或在线流再次看到同一人或已教学整图场景时，服务端输出低频 memory semantic event，CLI 只把 memory 事件投影成 Botified frame。`familiar_unknown_present` 作为 supporting contract/report gate 覆盖，不作为 core GA 发布能力宣称。人像示教必须覆盖两类典型线下交互场景：用户说“请记住我”时，服务端只使用新鲜、可互动、可见的 `active_interaction_target` 解析自己，attention target 只能作为派生 `active_interaction_target` 的输入；用户说“我介绍一下这位是 xxx”时，服务端先派生介绍人 A 的 `active_interaction_target`，再用 A 的手臂姿态解析被介绍人 B。region teaching 只作为测试-only fixture 验证或后续能力，不作为稳定 agent-facing GA capability。
...
- core GA gate 聚焦：`teach_person` self introduction、`teach_person` third-person introduction by arm pointing、known replay `known_person_present`、whole-scene `teach_scene` / `scene_activated`、ambiguous/no-write、object negative-only、非阻塞 10Hz/gaze。
- supporting contract/report gate 覆盖但不抢主线：`memory_context`、conversation summary / compact background、external user link、`familiar_unknown_present`、`merge-anonymous-person`、`correct-identity`、`resolve-target` dry-run 三态预览。
...
- 重复示教采用最小策略：高置信匹配到已知 person 且同名或同 external ref 时只更新 metadata；高置信匹配到已知 person 但不同名时在 teach 写入阶段返回 `conflict` outcome / `error_code` / HTTP 409，不创建新 person；匹配 anonymous profile 时走显式 link/merge path；不自动创建 duplicate。
...
`target.kind` 是正式 agent-facing API 形态；当前代码仍存在 `target.mode` 这类低层内部形态。本计划的前置实现任务是增加 kind-to-internal-target adapter / schema migration，把 stable `target.kind=person|scene|object` 转换为内部 resolver 可执行的目标结构；`region` 只在测试-only fixture 或后续能力路径中处理。完成该 adapter 前，`des.txt` 映射出的 payload 不能被当作当前代码已可直接执行。
...
- 如果现有 `pic_teach_person` 没有稳定窗口，必须新增或选择专门的多人介绍场景作为 auxiliary local smoke scene；不能用 fixture 直接指定 B 来替代 local smoke。auxiliary scene 与“全 15 个 `val-data` 场景 replay”并列报告，不改变全 15 口径。
- 如果 auxiliary scene 被正式加入 `val-data/`，报告中的场景数必须自动变成 16+，列出新增场景名、用途和是否纳入全量 replay；如果它只作为外部 auxiliary fixture，则报告必须单独列在 `auxiliary_local_smoke_scenes[]`，不把它伪装成 15 个基础场景之一。
...
- `pic_teach_me` 映射为 `target.kind=person`、`target.intent=self_introduction`、`target.referent_text=我`，`profile.display_name=小李飞刀`，`profile.description` 保留原句或测试备注。它是典型线下交互场景中的用户自我介绍、让机器人记住自己的正向用例，不是合成治理用例。server 只接受新鲜、可互动、可见的 `active_interaction_target`；attention target 只能作为派生输入，否则返回 `ambiguous` 或 `not_found`，不走 raw attention target 或普通候选 fallback。
- `pic_teach_person` 映射为 `target.kind=person`、`target.intent=third_person_introduction`、`target.referent_text=这位/彭刚`，`profile.display_name=彭刚`，`profile.description` 保留原句或测试备注。server 必须用 `active_interaction_target` 作为介绍人 A，并用 pose pointing resolver 解析被介绍人 B；没有 A、A 不稳定或 B 不确定时返回 `ambiguous`，不写库。
- `pic_teach_scene_galbot` 映射为 `target.kind=scene`、`target.intent=teach_scene`，`memory.title=银河通用办公室`，`memory.description` 保留原句。
- 本发布不从 `des.txt` 派生 GA region payload。需要验证实验性 region path 时，可以在测试-only fixture 模式发送 `target.kind=region`、`target.intent=teach_region`，但 bbox/point/test hint 主路径必须通过 runner-only envelope 提供；没有可靠 visual region hint 时预期返回 `ambiguous` 或 `not_found`，不写库。
- `pic_teach_item_phone` 映射为 `target.kind=object`、`target.intent=teach_object`，`target.referent_text=手机`。预期结果唯一为 `status=not_found` 且带 `error_code=unsupported_target_kind` / reason；不写 object memory，也不降级为 scene/region memory；report 中记录 `expected_negative=true` 和拒绝原因。
...
- teach person、self introduction、third-person introduction pose pointing、known person、whole-scene teach/activation、familiar、merge、correction、summary、external user link、ambiguity、negative 和 object unsupported 合同正确。
...
机器断言分为 core GA gate 和 supporting contract gate。core gate 失败时不能宣称 GA；supporting gate 失败时不能宣称对应 supporting contract 已 handoff。

core GA gate：

- 全 15 个 `val-data` 场景都被 replay，报告列出每个场景结果；如 auxiliary scene 加入 `val-data/`，报告场景数自动变成 16+ 并列出新增项，如仅作为外部 auxiliary local smoke scene 则单独列出且不改变 15 场景口径。
...
supporting contract/report gate：
...
- duplicate teach strategy 通过 API/report 断言：同名/同 external ref 高置信已有 person 只更新 metadata；高置信同脸不同名在 teach 写入阶段返回 `conflict` outcome / `error_code` / HTTP 409，不新建；anonymous match 走 link/merge path；不自动创建 duplicate。
- `familiar_unknown_present`、merge anonymous、correct identity、conversation summary / compact background、external user link、`memory_context` 字段在 fake backend 下走同一 API/store/retriever/event/CLI projection 路径；local backend 可用固定样本或构造输入证明路径，不要求每个分支都成为重型真实模型 gate。`familiar_unknown_present` 是 supporting contract gate，不作为 core GA 发布能力宣称。
...
15. 确认 duplicate teach strategy：同名/同 external ref 更新 metadata；同脸不同名在 teach 写入阶段返回 `conflict` outcome / `error_code` / HTTP 409，不新建；anonymous match 走 link/merge path；不自动 duplicate。
...
20. 新增 GA runner 或扩展现有 runner，保留原 `run_memory_e2e` fake/synthetic 快速回归；不要把旧 `run_val_data_e2e` 7 场景当作 memory/teaching 全量 gate。
