...
本阶段目标是把“记忆和示教”作为稳定可发布能力交付：用户通过 agent 明确示教当前画面中的人或整图场景，服务端保存记忆；后续 replay 或在线流再次看到同一人或已教学整图场景时，服务端输出低频 memory semantic event，CLI 只把 memory 事件投影成 Botified frame。`familiar_unknown_present` 作为 supporting contract/report gate 覆盖，不作为 core GA 发布能力宣称。人像示教必须覆盖两类真实店面场景：用户说“请记住我”时，服务端只使用新鲜、可互动、可见的 `active_interaction_target` 解析自己，attention target 只能作为派生 `active_interaction_target` 的输入；用户说“我介绍一下这位是 xxx”时，服务端先派生介绍人 A 的 `active_interaction_target`，再用 A 的手臂姿态解析被介绍人 B。region teaching 只作为测试-only fixture 验证或后续能力，不作为稳定 agent-facing GA capability。
...
- 重复示教采用最小策略：高置信匹配到已知 person 且同名或同 external ref 时只更新 metadata；高置信匹配到已知 person 但不同名时返回 `conflict`，不创建新 person；匹配 anonymous profile 时走显式 link/merge path；不自动创建 duplicate。
...
- `pic_teach_me` 映射为 `target.kind=person`、`target.intent=self_introduction`、`target.referent_text=我`，`profile.display_name=小李飞刀`，`profile.description` 保留原句或测试备注。它是实体店门口用户自我介绍、让机器人记住自己的正向用例，不是合成治理用例。server 只接受新鲜、可互动、可见的 `active_interaction_target`；attention target 只能作为派生输入，否则返回 `ambiguous` 或 `not_found`，不走 raw attention target 或普通候选 fallback。
...
- duplicate teach strategy 通过 API/report 断言：同名/同 external ref 高置信已有 person 只更新 metadata；高置信同脸不同名返回 `conflict` 不新建；anonymous match 走 link/merge path；不自动创建 duplicate。
