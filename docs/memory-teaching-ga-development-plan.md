...
- 如保留 region path，正式 agent-facing request 仍只提供 `target.kind`、`referent_text` / `intent`、`camera` 和 memory metadata；bbox、point、test hint 等只能来自 runner-only envelope 或 server debug/test channel。
...
- server 内部可以使用 attention target、scene context、tracks、bbox、pose/keypoints、region hints 和 runner/debug fixture 解析候选；这些不是 agent-facing REST contract 字段。
- `track_id`、`bbox`、`point_uv`、`test_hint`、`source_scene`、`source_frame` 只能出现在 runner-only envelope、report 或 server debug/test channel，不能进入稳定 REST contract 示例。
...
agent 只负责把用户语言整理成目标意图，visual-events 服务负责把目标意图和当前画面关联起来。系统可以使用 attention target、track、bbox、pose/keypoints、场景状态、region hints 和 runner/debug fixture 做隐式解析，但不能静默猜错并写库。
...
- runner 可以在 request body 外维护 envelope，或通过 server debug/test channel 提供固定 fixture；这些字段必须在 report 中标记为 runner/debug 输入，不能伪装成 agent 提供的字段。
...
- `person` 候选解析可以使用单人可见、track 稳定性、中心/近距关系、pose/手臂方向 tie-break 和 runner/debug fixture。多人候选分数接近时必须返回 `ambiguous`，不能选择最大的人或最近的人强行写库。
- `scene` 直接解析为整图。
- `region` 只支持实验性最小矩形区域：由可靠 region hint、runner fixture 或 server debug/test channel 解析出来。没有可靠可解析区域时返回 `ambiguous` / `not_found`，不写库。
...
5. 确保 intent-to-target resolver 支持稳定输入 `target.kind=person|scene|region|object`，输出 `resolved`、`ambiguous`、`not_found` 或 `unsupported_target_kind`；`resolve-target` 和 teach API 使用同一套 resolver，但 teach 不能复用 dry-run 结果。
6. 正式 REST schema 只允许 agent-facing 字段：`camera`、`target.kind`、`target.referent_text` / `target.intent`、profile/memory metadata。`track_id`、`bbox`、`point_uv`、`test_hint`、`source_scene`、`source_frame` 只能在 runner-only envelope、report 或 server debug/test channel 出现。
7. 保留 region path 时按实验能力处理：
   - 不作为 GA 主线或硬 gate。
   - 正式 request 不接收 bbox/point/test hint 等低层字段。
   - 没有可靠 region hint / runner fixture / debug channel 输入时返回 `ambiguous` 或 `not_found`，不写库。
...
- runner-only envelope 与正式 REST request body 必须硬隔离：`source_scene`、`source_frame`、`track_id`、`bbox`、`point_uv`、`test_hint` 只能存在于 envelope、report 或 server debug/test channel，不能进入稳定 REST contract 示例，也不能伪装成 agent 提供的字段。
...
- `source_scene`、`source_frame`、`track_id`、`bbox`、`point_uv`、`test_hint` 只能放在 runner-only envelope、report 或 server debug/test channel；不得进入 agent-facing payload 示例。
...
- `pic_teach_person` 映射为 `target.kind=person`，`target.referent_text=彭刚/这个人`，`profile.display_name=彭刚`，`profile.description` 保留原句或测试备注。server 可以用 attention、单人/多人候选、pose tie-break 或 runner/debug fixture 解析；不确定时返回 `ambiguous`。
- `pic_teach_scene_galbot` 映射为 `target.kind=scene`，`memory.title=银河通用办公室`，`memory.description` 保留原句。
- 本发布不从 `des.txt` 派生 GA region payload。需要验证实验性 region path 时，可以发送 `target.kind=region` 的正式 request，但 bbox/point/test hint 必须通过 runner-only envelope 或 server debug/test channel 提供；没有可靠 visual region hint 时预期返回 `ambiguous` 或 `not_found`，不写库。
...
- 如果实现，使用 `pic_teach_scene_galbot` 或选定稳定场景，通过 runner-only envelope 或 server debug/test channel 提供固定 visual region hint；正式 request body 不携带 bbox/point/test hint。
...
- 低相似度、低质量、无新鲜 frame、过期 runner/debug target fixture 都必须返回明确错误或无事件。
...
- target source 必须固定：使用稳定 request 输入和固定 recent frame；如需低层辅助，只能通过 runner-only envelope 或 server debug/test channel 提供，并在 report 中标为非 agent-facing 输入。
...
- runner-only envelope 摘要：`source_scene`、固定 teach frame、target fixture/debug 输入、resolver evidence；这些不得混入正式 request body。
...
- 正式 REST contract 和示例只包含 `camera`、`target.kind`、`target.referent_text` / `target.intent`、profile/memory metadata；`track_id`、`bbox`、`point_uv`、`test_hint`、`source_scene`、`source_frame` 只存在于 runner-only envelope、report 或 server debug/test channel。
...
6. 确认 intent-to-target resolver 接受 `target.kind=person|scene|region|object`，输出 `resolved`、`ambiguous`、`not_found` 或 `unsupported_target_kind`，且失败状态不写库。
