...
- 旧输出目录和旧脚本名混入了阶段性开发心智，会让人误以为 demo 不是正式真实模型结果。

本计划的目标是把 demo 收敛成两个稳定入口，默认全部使用真实模型。这里不做旧命令兼容，不保留旧输出命名作为用户入口，不为了历史 artifact 增加绕过参数。测试可以使用 test double，但它们不属于产品 demo 心智。
...
报告文件只描述本次 demo 事实，不承担测试门禁、发布审计或验收签核职责。

本计划采用 clean break：开发完成后，README 和 active docs 只指向这两个入口。旧的组合命令和旧输出目录不再作为可选做法保留。
...
旧测试/阶段性入口不再作为公开入口，相关文件直接归档、重命名或删除。README 和 active docs 不列旧做法的替代命令。

内部实现可以复用已有函数，但用户不需要知道这些函数名。开发时可以重命名或删除旧的公开 wrapper；不为旧 CLI 参数、旧目录结构或旧 artifact 输入提供兼容承诺。
...
3. `report.json` 和 HTML 顶部必须显示本次使用的模型路径和 backend。
4. `report.json` 必须包含：
...
3. 写出 wrapped `visual_state.jsonl` 到 `artifacts/demo/visual/visual_state.jsonl`。
...
公开 demo 不支持外部 server、历史 JSONL 或历史 replay artifact 输入。需要定位底层 replay 问题时，开发者直接使用 `tools/replay_val_data.py` 或模块级 helper，不把这些模式塞回 demo CLI。
...
face crop、face score、match score 放入折叠调试区或 raw 附件，不作为首页主视觉信息。
...
两个 demo 只要求根目录有 `index.html` 和 `report.json`。内部图片、scene 页面、states、crops、raw JSON 的目录结构分别沿用现有 renderer，不强行统一，避免无收益迁移。用户打开 `index.html` 不需要理解内部目录。

禁止把新结果写入 `val-data/`、`runtime/models/` 或 Git tracked 路径。
...
- legacy 文档继续留在 `docs/legacy/`，但不再作为 active demo 指南。

### Step 2: Visual evidence 默认真实模型

- 新增或改名为 `tools/run_visual_demo.py`，作为唯一 visual demo 入口。
- 默认启动临时本地 server 并在线跑全量 replay。这是一次破坏式语义收口，不保留离线 artifact 输入 CLI 兼容。
...
- 公开入口不暴露 backend 选择、source artifact 或阶段性开发模式。
- 默认模型路径固定为：
...
- 旧 memory evidence 公开脚本不再作为入口。开发团队可以删除、重命名或移动到 legacy/internal 位置；不需要保留兼容 alias。

### Step 4: Memory renderer 补齐真实模型信息

- HTML 顶部展示模型和 backend。
...
- 公开 memory evidence 不支持读取任意旧 source artifact；它自己生成本次真实模型结果并渲染。
- 旧 artifact 目录名不作为 active demo 入口，也不提供兼容迁移逻辑。

### Step 6: 输出清理

- 两个生成器运行前清理自己管理的文件和目录，避免旧 scene/image 残留。
- 默认输出目录固定时，可以直接重建 `artifacts/demo/visual/` 或 `artifacts/demo/memory/`。
- 如果用户显式传 `--out`，只清理该目录内工具管理的固定 children，例如 `images/`、`scenes/`、`crops/`、`raw/`、`index.html`、`report.json`。
- 工具不清空整个 `artifacts/`。
...
- memory CLI 不提供 source artifact / backend 选择参数。
- visual CLI 默认走真实在线 replay orchestration。
- 两个 CLI 的输出路径不能位于 `val-data/` 内。
...
- memory report 的 embedding model 与本地模型 manifest 一致。
...
- active docs 不再把旧测试/阶段性入口作为 demo 主路径。
- 旧公开命令已从 README 和 active docs 移除；无需兼容 alias。
...
本计划按 KISS / DRY / YAGNI 收敛为：两个公开入口、两个固定输出目录、默认真实模型、清理旧用户心智、内部实现复用现有模块。
