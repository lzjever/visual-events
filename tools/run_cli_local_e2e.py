...
def _runtime_provenance_report_for_failure(args: argparse.Namespace) -> dict[str, Any]:
    try:
        return _collect_runtime_provenance(
        ...
        return {
            "server_bin": os.fspath(_resolve_path(args.server_bin)),
            "cli_bin": os.fspath(_resolve_path(args.cli_bin)),
            ...
            **_empty_runtime_distribution_report(),
            "server_script_sha256": None,
            "cli_script_sha256": None,
            "runtime_hash": None,
            "config_path": os.fspath(_resolve_path(args.server_config))
...
def _collect_runtime_provenance(
    *,
    ...
    distribution_report, distribution_failures = _runtime_distribution_provenance_report(
...
def _project_distribution_contract() -> tuple[str, str, dict[str, str]]:
    try:
        ...
        raise PreflightError("pyproject project name/version unavailable") from exc
    if not isinstance(name, str) or not isinstance(version, str):
        raise PreflightError("pyproject project name/version must be strings")
    scripts = project.get("scripts")
    if not isinstance(scripts, dict):
        raise PreflightError("pyproject project scripts unavailable")
    expected_scripts: dict[str, str] = {}
    for script_name in (SERVER_SCRIPT_NAME, CLI_SCRIPT_NAME):
        target = scripts.get(script_name)
        if not isinstance(target, str):
            raise PreflightError(f"pyproject script missing: {script_name}")
...
def _empty_runtime_distribution_report() -> dict[str, Any]:
    return {
        "wheel_name": None,
        "wheel_version": None,
        "dist_info_dir": None,
...
def _runtime_distribution_provenance_report(
    *,
    ...
    empty_report = _empty_runtime_distribution_report()
...
def _runtime_record_report(
    *,
    ...
    for role, script_path in script_paths.items():
        row = _runtime_record_row_for_path(
            rows,
...
def _runtime_record_row_for_path(
    rows: list[list[str]],
    *,
    expected_path: Path,
    site_packages: Path,
...
def _runtime_provenance_flat_aliases(
    ...
    *,
    ...
    nested_report = _runtime_provenance_report_with_exit_codes(
        runtime_report,
        server_exit_code=server_exit_code,
        cli_exit_code=cli_exit_code,
    )
